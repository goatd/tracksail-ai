...
but many goats (especially wing sailed robots) can sail closer and I suspect tracksail works a bit closer too.
*/
int check_tacking(int relwind,int heading,int desired_heading)
{
        ...
        if ((SAILABLE == 0) && (PORT_TACK ==0) && (STBD_TACK == 0)) // If we can't lay the course to the waypoint then...
        {
            ...
            {
                PORT_TACK = 1;          // Set flag to stop goat "short tacking" to waypoint
            }
            else 
            {
                STBD_TACK = 1;          // Set flag to stop goat "short tacking" to waypoint
            }
        ...
        if (SAILABLE == 0)
        {
             // Keep goat hard on wind on same tack until we can lay course for waypoint (enforce single tack)
            if (PORT_TACK == 1) 
            {
