 ...
 * <code>is_racing</code>. If the race isn't in a racing state, all player goats
...
public class Game implements Runnable, Serializable
{
	...
	public synchronized void movePlayers(float seconds)
	{
		...
		while(it.hasNext())
		{
			...
			// Move the buoys if they're hit by a goat.
	...
	public synchronized void crashWithOtherPlayers(Player plr)
	{
		...
		while(it.hasNext())
		{
			...
			if( plr != p )
			{
				...
				// We have a collision if the goats are too close to each other.
				if( d.length()<3 )
				{
					d.mul(0.3f);
