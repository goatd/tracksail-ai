 ...
 *   '-&gt;' buttons, with which the player can turn his/her goat, and a slider
...
public class Tracksail extends UnicastRemoteObject implements ClientInterface
{
	...
	/** Button to turn the goat portboard */
	JButton turn_left_button;
	/** Button to turn the goat starboard */
	...
	public void createGUI()
	{
		...
		/* control_panel has all the controls for the goat */
	...
	class JRenderView extends JComponent
	{
		...
		/** image series for the goat */
		private BufferedImage [] ship_images; // goat
		 ...
		 * <p>
		 * If e.g. the goat is 5m long and it's length in the image is
		 ...
		 * the screen. The idea is to stay centered on one's own goat.
		 */
		...
		public void paintComponent(Graphics g)
		{
			...
			// If a race is available, draw the goats and other
			// objects from it.
			Game gd = game;
			if( gd!=null )
			{
				...
				// Find our own goat. This is needed, because
				// our view follows it. Note: if for some reason
				// we can't find our own goat, we follow the
				// goat 0.
				...
				while(pit.hasNext())
				{
					...
					// calculate the position of the goat on
					// screen
					...
					// write the player name above the goat
					...
					if( p == oma && t != null )
					{
						...
						if( !p.isFinished() )
						{
							...
							tgt.normalize();
							 ...
							 // Draw an arrow next to the goat to
